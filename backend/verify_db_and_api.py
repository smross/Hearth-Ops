...
def test_database_and_pin_hashing():
    ...
    with get_db_connection() as conn:
        ...
        for user in users:
            ...
            if user['name'] == 'Shawn':
                shawn_found = True
                # Shawn's expected PIN is 3157
                expected_hash = hash_pin("3157")
                if user['pin_hash'] == expected_hash:
                    print("   [PASS] PIN hash matches expected SHA-256 hash for Shawn.")
                else:
                    print(f"   [FAIL] Hash mismatch for Shawn. Got {user['pin_hash']}, expected {expected_hash}")
                    sys.exit(1)
        
        if not shawn_found:
            print("Error: Shawn not found in database users.")
            sys.exit(1)
...
def test_api_routing_internally():
    ...
    try:
        ...
        response = client.post("/api/verify", json={"name": "Shawn", "pin": "3157"})
        if response.status_code == 200:
            user_data = response.json()
            print(f"[PASS] TestClient POST /api/verify succeeded for Shawn (ID: {user_data.get('id')}).")
        else:
            print(f"[FAIL] TestClient POST /api/verify failed for Shawn: {response.status_code} {response.text}")
            sys.exit(1)
            
        # Test verify endpoint (invalid pin)
        response = client.post("/api/verify", json={"name": "Shawn", "pin": "9999"})
        ...
        try:
            req = urllib.request.Request(
                ...
                data=json.dumps({"name": "Shawn", "pin": "3157"}).encode(),
                headers={"Content-Type": "application/json"}
            )
            with urllib.request.urlopen(req) as r:
                res = json.loads(r.read().decode())
                print(f"[PASS] urllib POST /api/verify succeeded for Shawn (ID: {res.get('id')}).")
        except Exception as ex:
